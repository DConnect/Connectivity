(function ( $ ){
	...
	*/
	...
	var defaultKey		= 'puBRA5t636E6WhJXi9xfa3xWP5Dzu4kA6x8vUh6UqNDCwDnt', // Unique master Xively API key to be used as a default
		defaultFeeds	= [1509467466], // Comma separated array of Xively Feed ID numbers
		...
		dataInterval	= 10800, // Default interval for data to be displayed (in seconds)
		dataColor		= '024314', // CSS HEX value of color to represent data (omit leading #)
							...
							xively.datastream.history(feedId, datastream.id, {duration: duration, interval: interval, limit: 1000}, function(datastreamData) {
								...
								if(datastreamData.datapoints) {
									...
									var graph = new Rickshaw.Graph( {
										...
										height: 200,
										renderer: 'line',
										min: parseFloat(datastream.min_value) - 8*(parseFloat(datastream.max_value) - parseFloat(datastream.min_value)),
										max: parseFloat(datastream.max_value) + 8*(parseFloat(datastream.max_value) - parseFloat(datastream.min_value)),
										padding: {
											top: 0.02,
											right: 0.02,
											bottom: 0.02,
